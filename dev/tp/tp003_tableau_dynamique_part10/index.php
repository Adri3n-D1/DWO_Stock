    ...
    <table class="table table-striped">
        ...
        <?php foreach ($result as $row): ?>
            <tr>
                ...
                <td><?= $row['price'] ?></td>
                <td><?= $row['city'] ?></td>
                <td><span class="address-label"><?= $row['address'] ?></span><textarea id="<?= $row['id'] ?>"class="adress-text-area"><?= $row['address'] ?></textarea></td>
                <td>
...
<script>
    ...
    function sendRequest(id, value) {
        /* Send a request to the server to modify an address in the database */
        
        let xhr = new XMLHttpRequest();
        // Requete ouverte de type GET
        xhr.open('POST', 'serveur.php');
        // Réponse au format json attendu
        xhr.responseType = 'text';

        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        // Requete envoyée
        xhr.send(`id=${id}&address=${value}`);
        // Evenement onload etabli
