...
if (isset($_POST['city-filter'])) {
    $city = htmlentities($_POST['city-filter']);
    $sql = 'SELECT COUNT(*) as nb FROM products WHERE city LIKE :city';
}
else {
...
echo json_encode((int)$request->fetch(PDO::FETCH_ASSOC)['nb']);
...
//     $query = "SELECT * FROM products ORDER BY id DESC LIMIT 20 OFFSET :offset";
